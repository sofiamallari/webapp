...
<?php
	...
	if(isset($_SESSION['status'])){
		...
		if(mysqli_num_rows($result)>=1){	
			echo "<p id='errorreg'>Record already exists<br></p>";
			$noError = false;
		}
		
		if((preg_match("/^[a-zA-Z ]+$/",$fname))==0){
				echo "<p id='errorreg'>Only letters and white space allowed for Firstname<br></p>";
				$noError = false;
		}else if((preg_match("/^[a-zA-Z ]+$/",$lname))==0){
				echo "<p id='errorreg'>Only letters and white space allowed for Lastname<br></p>";
				$noError = false;
		}else if((preg_match("/^[a-zA-Z ]+$/",$mname))==0){
				echo "<p id='errorreg'>Only letters and white space allowed for Middlename<br></p>";
				$noError = false;
		}else if((preg_match('/\./', $email)) == 0){
			echo "<p id='errorreg'>Invalid Email Address<br></p>";
		}
		...
		echo "<p id='errorreg'>No rows found</p>";
		exit;
		}
